...
<div class="box-body">
    <div class="row">
      ...
      <div class="form-group col-xs-12">
			...
			<label>
			<?php
			/*<input type="checkbox" class="icheck" name="user_type[]" value="<?php echo $group['id'];?>"<?php echo (in_array($group['id'],$cg)) ? ' checked="checked"' : ''; ?> /> <?php echo htmlspecialchars($group['description'],ENT_QUOTES,'UTF-8');?></label>
			*/?>
			<input type="checkbox" class="icheck" name="user_type[]" value="<?php echo $group->id;?>"<?php echo (in_array($group->id,$cg)) ? ' checked="checked"' : ''; ?> /> <?php echo htmlspecialchars($group->description,ENT_QUOTES,'UTF-8');?></label>
			</div>
			<?php endforeach?>
