...
function turn_on_icheck(){
	...
	$('a.toggle-swal_select').bind('click',function(e) {
		e.preventDefault();
		var url = $(this).attr('href');
		$.get(url).done(function(response) {
			var data = $.parseJSON(response);
			...
			swal({
				...
				inputValidator: function (value) {
					return new Promise(function (resolve, reject) {
						if (value === '') {
							reject('Pilih rombongan belajar terlebih dahulu');
						} else {
							resolve();
						}
					});
					...
					$.ajax({
						url: url,
						type: 'post',
						data: {rombongan_belajar_id:result},
						success: function(response){
							var result = $.parseJSON(response);
							console.log(result);
...
$(function() {
	var url_auto = $("#autocomplete").data('url');
	var options = {
		url: function(siswa) {
			return url_auto;
		},
			...
			"fnServerData": function ( sSource, aoData, fnCallback ) {
					$.ajax( {
						...
						"success": function(data, textStatus, jqXHR) {
							if(data.iTotalRecords > 0 && kompetensi){
								$('#filter_tingkat').show();
							} else {
								$('#filter_tingkat').hide();
							}
			...
			$('#datatable').dataTable({
				...
				"fnServerData": function ( sSource, aoData, fnCallback ) {
					$.ajax( {
						...
						"data": aoData,
						"success": function(data, textStatus, jqXHR) {
							if(data.iTotalRecords > 0 && tingkat){
								$('#filter_rombel').show();
	...
	$('#rombel').change(function(){
		...
		if(query == 'rapor'){
			...
			get = 'kurikulum';
		//} else if(query == 'analisis_individu'){
			//get = 'undefined';
		} else {
			get = 'mapel';
		}
	...
	$('#jurusan_sp_id').change(function(){
		...
		$.ajax({
			url: '<?php echo site_url('admin/ajax/get_kurikulum');?>',
			type: 'post',
			data: $("form").serialize(),
			success: function(response){
				var data = $.parseJSON(response);
	...
	$('#fileupload').fileupload({
		...
		swal({title:data.result.title,type:data.result.type,html:data.result.text}).then(function() {
			if(typeof(data.result.redirect) != "undefined" && data.result.redirect !== null) {
