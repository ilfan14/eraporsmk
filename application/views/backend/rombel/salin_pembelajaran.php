...
<div class="row">
	...
	<?php
	if(!$data_rombel->jurusan_sp_id){
		$get_kurikulum = $this->kurikulum->find_by_kurikulum_id($data_rombel->kurikulum_id);
		if($get_kurikulum){
			$get_jurusan = $this->jurusan->find_by_jurusan_id($get_kurikulum->jurusan_id);
			if($get_jurusan){
				$this->rombongan_belajar->update($data_rombel->id, array('jurusan_sp_id' => $get_jurusan->jurusan_id));
				$find_jur_sp = $this->jurusan_sp->find_by_kurikulum_id($get_jurusan->jurusan_id);
				if(!$find_jur_sp){
					$insert_jurusan_sp = array(
						'sekolah_id' 	=> $data_rombel->sekolah_id,
						'kurikulum_id' 	=> $get_jurusan->jurusan_id,
						'semester_id'	=> $data_rombel->semester_id,
					);
					$this->jurusan_sp->insert($insert_jurusan_sp);
				}
			}
		}
	}
	$all_pembelajaran = $this->pembelajaran->find_all("rombongan_belajar_id = $data_rombel->id AND mata_pelajaran_id NOT IN(SELECT id FROM ref_mata_pelajaran)");
	if($all_pembelajaran){
		foreach($all_pembelajaran as $pembelajaran){
			if(!is_numeric($pembelajaran->mata_pelajaran_id)){
				$query_mapel = $this->db->get_where('data_mapels', array('id_mapel' => $pembelajaran->mata_pelajaran_id))->row();
				if($query_mapel){
					$find_ref_mapel = $this->mata_pelajaran->find_by_id_nasional($query_mapel->id_mapel_nas);
					if($find_ref_mapel){
						$this->pembelajaran->update($pembelajaran->id, array('mata_pelajaran_id' => $find_ref_mapel->id));
					}
				}
			}
		}
	...
	<?php $i=1;
		...
		foreach($all_mapel as $mapel){
			//$get_guru_mapel_prev = get_guru_mapel($ajaran_id,$id_rombel_prev,$mapel->mata_pelajaran_id,'id');
			//test($test);
			$query = 'kurikulum';
	?>
		<tr>
			...
			<td>
				<input type="hidden" class="nama_mapel_alias" name="nama_mapel_alias" value="<?php echo get_nama_mapel_alias($mapel->mata_pelajaran_id); ?>" />		
			...
			<td>
				<input type="hidden" class="guru" name="guru" value="<?php echo get_guru_mapel($ajaran_id,$id_rombel_prev,$mapel->mata_pelajaran_id,'id'); ?>" />
				<a class="guru" href="javascript:void(0)" id="country" data-type="select2" data-name="guru" data-value="<?php echo get_guru_mapel($ajaran_id,$id_rombel_prev,$mapel->mata_pelajaran_id, 'id');?>" title="Pilih Guru"></a>
			</td>
			<td>
