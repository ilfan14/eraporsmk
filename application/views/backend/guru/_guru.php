				...
				<div class="row">
					<div class="col-xs-6"> 
						<div class="form-group col-xs-12">
							...
							<?php 
							echo form_hidden('sekolah_id', $sekolah_id);
							echo form_input($nama);?>
						</div>
						<div class="form-group col-xs-12">
							<label>Jenis Kelamin</label>
							<select name="jenis_kelamin" class="form-control required select2">
						...
						<div class="form-group col-xs-12">
							...
							<select name="status_kepegawaian_id" class="form-control required select2">
								<option value="">== Pilih Status Kepegawaian ==</option>
							<?php 
							$get_status_kepegawaian = status_kepegawaian();
							foreach ($get_status_kepegawaian as $status){?>
								<option value="<?php echo $status->id;?>"<?php echo ($status->id == $status_kepegawaian_id) ? ' selected' : '' ; ?>><?php echo htmlspecialchars($status->nama,ENT_QUOTES,'UTF-8');?></option>
							<?php } ?>
							</select>
						</div>
						<div class="form-group col-xs-12">
							<label>Jenis PTK</label>
							<select name="jenis_ptk" class="form-control required select2">
								<option value="">== Pilih Jenis PTK ==</option>
							<?php 
							$get_jenis_ptk = jenis_ptk();
							foreach ($get_jenis_ptk as $jenis){?>
								<option value="<?php echo $jenis->id;?>"<?php echo ($jenis->id == $jenis_ptk) ? ' selected' : '' ; ?>><?php echo htmlspecialchars($jenis->nama,ENT_QUOTES,'UTF-8');?></option>
							<?php } ?>
							</select>
						</div>
						<div class="form-group col-xs-12">
							<label>Agama</label>
							<select name="agama_id" class="form-control required select2">
								<option value="">== Pilih Agama ==</option>							
							<?php 
							$agamas = agama();
							foreach ($agamas as $agama):?>
								<option value="<?php echo $agama->id;?>"<?php echo ($agama->id == $agama_id) ? ' selected' : '' ; ?>><?php echo htmlspecialchars($agama->nama,ENT_QUOTES,'UTF-8');?></option>
							<?php endforeach?>
							</select>
						</div>
						</div>
