...
class Users extends Backend_Controller {
	...
	public function edit($id){
		...
		//test($groups);
		if($this->ion_auth->in_group('siswa', $user->id)){
			$groups=$this->group->get_many(array(4));
		}
		if($this->ion_auth->in_group('guru', $user->id)){
			$groups=$this->group->get_many(array(2,3,6));
		}
		if($this->ion_auth->in_group('waka', $user->id)){
			$groups=$this->group->get_many(array(2,3,6));
		}
		...
		if (isset($_POST) && !empty($_POST)){
			...
			if ($this->form_validation->run() === TRUE){
				...
				redirect('admin/config/users/', 'refresh');
			}
		}
	...
	public function create(){
		...
		$this->form_validation->set_rules('username', 'Nama Operator', 'required');
	...
	public function delete($id){
		...
		if($user->delete()){
			$status['type'] = 'success';
			$status['text'] = 'Data User berhasil dihapus';
			$status['title'] = 'Data Terhapus!';
		} else {
			$status['type'] = 'error';
			$status['text'] = 'Data User tidak terhapus';
			$status['title'] = 'Gagal!';
		}
