	...
	public function index(){
		...
		if($highestColumn == 'AD') { // Import data siswa
			...
			for ($row = 2; $row <= $highestRow; ++ $row) {
				...
				$masukkan[] = array_merge($data_sekolah_id,$flat,$active,$photo,$password,$petugas);
			}
		...
		foreach($masukkan as $k=>$v){
			...
			if(!$a){
				...
				$username 	= $v['nama'];
				$password 	= $v['password'];
				$email		= ($v['email'] ? $v['email'] : GenerateEmail().'@cybereducation.co.id');
				...
				if($user_id){
					...
					if($rombel){
						foreach($rombel as $r){ 
							$attributes = array('ajaran_id' => $ajaran->id, 'rombel_id' => $r->id, 'datasiswa_id' => $datasiswa->id);
						...
						$attributes = array('ajaran_id' => $ajaran->id, 'rombel_id' => $datarombelbawah->id, 'datasiswa_id' => $datasiswa->id);
						$anggota = Anggotarombel::create($attributes);
					}
