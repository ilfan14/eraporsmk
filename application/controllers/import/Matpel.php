<?php
class Matpel extends Backend_Controller {
	...
	public function index(){
		...
		if($highestColumn == 'F') { // Import data siswa
			...
			for ($row = 2; $row <= $highestRow; ++ $row) {
				...
				$masukkan[] = array_merge($flat, $petugas);
			}
		...
		foreach($masukkan as $k=>$v){
			$a = Matpelkomp::all(array('conditions' => array('kurikulum_id = ? AND id_mapel = ? AND kelas_X  = ? AND kelas_XI  = ? AND kelas_XII  = ? AND kelas_XIII = ?', $v['kurikulum_id'], $v['id_mapel'], $v['kelas_X'], $v['kelas_XI'], $v['kelas_XII'], $v['kelas_XIII'])));
			$sum+=count($a);
			if(!$a){
				if(!$v['kelas_X']){
					$v['kelas_X'] = 0;
				...
				$inser_data = Matpelkomp::create($v);
				if(!$inser_data){
