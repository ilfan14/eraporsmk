	...
	public function index(){
		...
		if($highestColumn == 'B') { // Import data guru
			...
			for ($row = 2; $row <= $highestRow; ++ $row) {
				...
				$petugas = array("petugas"=> $loggeduser->username, "ajaran_id" => $ajaran->id);
		...
		foreach($masukkan as $k=>$v){
			$a = Datarombel::all(array('conditions' => array('tingkat = ? AND nama = ? AND ajaran_id = ?', $v['tingkat'], $v['nama'], $ajaran->id)));
			$sum+=count($a);
			if(!$a){
