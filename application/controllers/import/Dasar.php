<?php
class Dasar extends Backend_Controller {
	...
	public function index(){
		...
		foreach($masukkan as $k=>$v){
			$a = Kd::all(array('conditions' => array('id_kompetensi = ? AND aspek = ? AND id_mapel  = ? AND kelas  = ? AND kompetensi_dasar = ?', $v['id_kompetensi'], $v['aspek'], $v['id_mapel'], $v['kelas'], $v['kompetensi_dasar'])));
			$sum+=count($a);
			if(!$a){
				$inser_data = Kd::create($v);
				if(!$inser_data){
